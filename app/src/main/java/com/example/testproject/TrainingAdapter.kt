...
class TrainingAdapter(private val getTrainedById: (String) -> String) :
    ...
    override fun onBindViewHolder(holder: TrainingViewHolder, position: Int) {
        ...
        if(item.type == TrainingType.TRAIN) {
            with(holder.itemView) {
                ...
                findViewById<TextView>(R.id.color).setBackgroundColor(
                    Color.parseColor(item.lesson?.color))
                item.lesson?.coachId.let {
                    findViewById<TextView>(R.id.name).text = getTrainedById(it)
                }
            }
        } else {
